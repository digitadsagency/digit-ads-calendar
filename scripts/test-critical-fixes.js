...
// Crear usuarios de prueba para verificar las correcciones críticas
async function createTestUsersForCriticalFixes() {
  try {
    console.log('🧪 Creando usuarios de prueba para verificar correcciones críticas...');
    ...
    // Crear dos usuarios para probar reservas duplicadas
    const testUsers = [
      {
    ...
    for (const reservation of reservations) {
      ...
      const reservationRow = [
        ...
        reservation.date,
        reservation.block,
        testUsers.find(u => u.email.includes(reservation.userId === userIds[0] ? 'usuario1' : 'usuario2'))?.name,
        testUsers.find(u => u.email.includes(reservation.userId === userIds[0] ? 'usuario1' : 'usuario2'))?.company,
        'Dirección de prueba',
        testUsers.find(u => u.email.includes(reservation.userId === userIds[0] ? 'usuario1' : 'usuario2'))?.whatsapp,
    ...
    console.log('');
    console.log('🔍 Para probar las correcciones críticas:');
    console.log('');
    console.log('1️⃣ VERIFICACIÓN DE DISPONIBILIDAD:');
    console.log('   - Ve a http://localhost:3000/client/login');
    console.log('   - Usa las credenciales del Usuario 1');
    console.log('   - Intenta hacer una reserva en la misma fecha y bloque que el Usuario 2');
    console.log('   - ✅ DEBERÍA mostrar: "El bloque de tarde ya está ocupado"');
    console.log('');
    console.log('2️⃣ CANCELACIONES Y CONTADOR:');
    ...
    console.log('     * Actualizar el contador de reservas del mes');
    console.log('     * Cambiar el estado a "Cancelada"');
    console.log('     * Permitir hacer una nueva reserva (ya que liberó el límite)');
    console.log('');
    console.log('3️⃣ LÍMITE MENSUAL:');
...
if (require.main === module) {
  createTestUsersForCriticalFixes();
}

module.exports = { createTestUsersForCriticalFixes };
